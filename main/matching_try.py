...
def fuzzy_match_partial(text, keywords, threshold=90):
    """
...
def match_text(text, keywords, threshold=90):
    """
    Match text against keywords - cascades through exact, fuzzy, then partial fuzzy match
    For partial fuzzy match, requires length ratio (smaller/bigger) > 0.5
    
    Args:
        text: String to match against keywords
        keywords: List of keyword strings to match
        threshold: Similarity threshold for fuzzy matching (0 to 100), default 90
        
    Returns:
        bool: True if text matches (exactly, fuzzily, or partial with length ratio check)
    """
    ...
    # Try fuzzy match
    if fuzzy_match(text, keywords, threshold):
        return True
    
    # Try fuzzy match partial with length ratio check
    if not text or not keywords:
        return False
    
    if fuzzy_match_partial(text,keywords,threshold):
        return True
